<template>
  <div class="col s12 m12 l9" id="vueSample">
    ...
    <div class="card" v-bind:style="{height:boardHeight+'px'}">
        ...
        <div class="cyan darken-4">
          <div id="wrapper">
            ...
            <form name="form2" style=" display : inline">
              <!-- <button id = "day" onclick="get_period_data('daily');" value="day" class="btn-flat-border">����</button>
								<button id = "week" onclick="get_period_data('weekly');" value="week" class="btn-flat-border">����</button>
								<button id = "month" onclick="get_period_data('monthly');" value="month" class="btn-flat-border">�</button>
              -->
              <button id="day" onclick="timeclick('daily');" value="day" class="btn-flat-border">Daily</button>
              <button
                id="week"
                onclick="timeclick('weekly');"
                value="week"
                class="btn-flat-border"
              >Weekly</button>
              <button
                id="month"
                onclick="timeclick('monthly');"
                value="month"
                class="btn-flat-border"
              >Monthly</button>
            </form>
            ...
            <div id="app">
              ...
              >
                ...
                >
                  <div v-if="!isOpen && values.length">{{ values.length }} options selected</div>
                </div>
              </multiselect>
              <!-- <pre>{{ value }}</pre> -->
      ...
      <div class="card-reveal grey darken-3 white-text" v-bind:style="revealStyleObj">
        <div class="row" v-bind:style="{height:revealContentHeight+'px'}"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    const channelName_FX_BTC_JPY_execution =
      "lightning_executions_" + "FX_BTC_JPY";
    ...
    let url_bitflyer =
      ...
      timing +
      "?fsym=BTC&e=bitflyerfx&tsym=JPY&limit=" +
      candletotal;
    let url_bitmex =
      "https://min-api.cryptocompare.com/data/histo" +
      timing +
      "?fsym=XBTUSD&e=bitmex&tsym=USD&limit=" +
      candletotal;
    ...
    function firebaseget() {
      ...
      async function getMarker() {
        ...
        snapshot.docs.map(doc => {
          ...
          if (
            doc.data().exchange == "bitmex_testnet" ||
            doc.data().exchange == "bitmex"
          ) {
    ...
    function soneki_first() {
      ...
      var ctx_soneki_bitmex = document
        .getElementById("myChart_soneki_bitmex")
        .getContext("2d");
      window.myChart_soneki_bitmex = new Chart(
        ctx_soneki_bitmex,
        config_soneki_bitmex
      );
      ...
      get_btc_currency_array(
        (_fsym = "XBTUSD"),
        (_e_type = "bitmex"),
        (_tsym = "USD"),
        (_limit = profit_visible_rows)
      );
    }
        ...
        methods: {
          ...
          onSelect(option, id) {
            ...
            for (var i in option) {
              ...
              strategy_selected_array.push(option[i].name); //����ľ��ɬ�פ���
            }
            delete window.myChart_soneki_bitmex.data.datasets;
    ...
    function get_btc_currency_array(
      _fsym = "XBTUSD",
      _e_type = "bitmex",
      _tsym = "USD",
      _limit = 500
    ...
    function get_premiumData() {
      ...
      let url =
        "https://rest.coinapi.io/v1/ohlcv/BITMEX_IDX_XBTUSDPI8H/history?apikey=" +
        api_key +
        "&period_id=8HRS&time_start=" +
        time_start +
        "Z&limit=10000";
    ...
    function calculateProfit(data, timePrice, type) {
      ...
      for (strategy in result) {
        ...
        for (timeInd in timePrice) {
          ...
          for (dataTimeInd in result[strategy]) {
            ...
            if (compareTimeStamp(result[strategy][dataTimeInd].t, time) > 0)
              continue;
            ...
            if (result[strategy][dataTimeInd].buysell == "buy") {
              ...
              profitVal =
                profitVal -
                result[strategy][dataTimeInd].lot *
                  (result[strategy][dataTimeInd].price / timePrice[timeInd].h -
                    1);
          ...
          temp[timeVal] =
            profitVal + feeVal + (temp[timeVal] ? temp[timeVal] : 0);
        ...
        total_by_strategy_bitmex.push({
          title: strategy,
          total_profit: strategyTotalProfit
        });
      ...
      for (temp_key in temp) {
        total_profit_bitmex.push({
          x: new Date(parseInt(temp_key)),
          y: temp[temp_key]
        });
      }
    ...
    function getFeeForTime(time, type) {
      ...
      if (type == "-bitmex") {
        ...
        while (feeUnixStp < unixStp) {
          ...
          i++;
          if (i == total_fee_bitmex.length) break;
        ...
        while (feeUnixStp < unixStp) {
          ...
          i++;
          if (i == total_fee_bitfly.length) break;
    ...
    function start_calc_func() {
      ...
      if (Bitmex_dealData.length > 0) {
        total_fee_bitmex = calculateBitmexFee(
          Bitmex_dealData,
          premium_data_array,
          currentTime
        );
        total_result_bitmex = calculateProfit(
          Bitmex_dealData,
          BTCprice_bitmex_array,
          "-bitmex"
        );
      }
      ...
      if (Bitfly_dealData.length > 0) {
        total_fee_bitfly = calculateBitflyFee(
          Bitfly_dealData,
          premium_data_array,
          currentTime
        );
        total_result_bitfly = getBitFlyProfit(
          Bitfly_dealData,
          BTCprice_bitfly_array
        );
      }
      ...
      console.log(
        "---------- Total Profit by Strategy for BitMex ------------"
      );
      console.log(total_by_strategy_bitmex);
      console.log(
        "---------- Total Profit by Strategy for BitFly ------------"
      );
    ...
    ***************************************************************/
    function compareTimeStamp(big, small, flag = false) {
      ...
      if (flag == false) return new Date(big) - next_t;
      else return new Date(big) - next_t + 3600000;
    }
    ...
    function getPremiumArray(time, currentTime) {
      ...
      for (index in premium_data_array) {
        if (
          start == 1 &&
          compareTimeStamp(currentTime, premium_data_array[index].t) > 0
        ) {
          filterData.push(premium_data_array[index].h * 100);
        }

        if (
          compareTimeStamp(premium_data_array[index].t, time) > 0 &&
        ...
        *******************************************************************************************************************************/
        ...
        if (premiumArray.length > 0) {
          for (premiumIndex in premiumArray) {
            var fr =
              premiumArray[premiumIndex] +
              Clamp(-0.05, interest_rate - premiumArray[premiumIndex], 0.05);
            funding_fee +=
              data[objIndex].lot * (nowBTCprice / data[objIndex].price) * fr;
          }
    ...
    function calculateBitflyFee(data, premiumData, currentTime) {
      ...
      for (objIndex in data) {
        var timeDiff = parseInt(
          compareTimeStamp(currentTime, data[objIndex].t) / 3600000
        );
        bitflyer_fee -=
          timeDiff * (data[objIndex].lot * data[objIndex].price * 0.04);
    ...
    function get_period_data(type) {
      ...
      for (bit_strategy_ind in total_result_bitmex) {
        ...
        while (data_time > beginningTimeStamp) {
          strategy_period_data.unshift({
            x: bit_strategy.res[i].x,
            y: bit_strategy.res[i].y
          });
          //strategy_period_data.unshift(bit_strategy.res[i]);
          temp[data_time] =
            (temp[data_time] ? temp[data_time] : 0) + bit_strategy.res[i].y;
          if (i > 0) {
        ...
        bitmex_period_data.push({
          title: bit_strategy.title,
          res: strategy_period_data
        });
      }
      for (temp_key in temp) {
        total_bitmex_period_data.unshift({
          x: new Date(parseInt(temp_key)),
          y: temp[temp_key]
        });
      }
      ...
      for (bit_strategy_ind in total_result_bitfly) {
        ...
        bitfly_period_data.push({
          title: bit_strategy.title,
          res: strategy_period_data
        });
      }
      for (temp_key in temp) {
        total_bitfly_period_data.unshift({
          x: new Date(parseInt(temp_key)),
          y: temp[temp_key]
        });
      }
      ...
      if (flag == "on") {
        ...
        console.log(bitmex_period_data);
        //list_to_chart_total(total_bitmex_period_data, "BitMEX��»��");
      ...
      final_profit =
        Math.round(
          total_bitmex_period_data[total_bitmex_period_data.length - 1].y * 100
        ) / 100;
          ...
          datalabels: {
            backgroundColor: function(context) {
              if (context.dataset._meta[0]) {
                return context.active
                  ? context.dataset._meta[0].dataset._model.backgroundColor
                  : "rgba(100, 100, 100, 0)";
              }
              if (context.dataset._meta[1]) {
                return context.active
                  ? context.dataset._meta[1].dataset._model.backgroundColor
                  : "rgba(100, 100, 100, 0)";
              }
              if (context.dataset._meta[2]) {
                return context.active
                  ? context.dataset._meta[2].dataset._model.backgroundColor
                  : "rgba(100, 100, 100, 0)";
              }
            },
            ...
            formatter: function(value, context) {
              ...
              return (
                ...
                "\n" +
                "$" +
                Math.round(context.dataset.data[context.dataIndex].y * 100) /
                  100
              );
